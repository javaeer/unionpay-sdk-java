...
/**
 * acpsdk配置文件acp_sdk.properties配置信息类
...
//	private String encryptTrackCertPath;
     ...
     * 配置文件中模数常量.
     */
    public static final String SDK_MODULUS ="acpsdk.modulus";

    /**
     * 配置文件中指数常量.
     */
    public static final String SDK_EXPONENT ="acpsdk.exponent";
...
//		}
        ...
        if (!SDKUtil.isEmpty(value)
                && SDKConstants.FALSE_STRING.equals(value.trim())) {
            this.ifValidateCNName = false;
        }

        value = pro.getProperty(SDK_IF_VALIDATE_REMOTE_CERT);
        if (!SDKUtil.isEmpty(value)
                && SDKConstants.TRUE_STRING.equals(value.trim())) {
        ...
        value = pro.getProperty(SDK_MODULUS);
        if (!SDKUtil.isEmpty(value)) {
            this.modulus = value.trim();
        }
        value = pro.getProperty(SDK_EXPONENT);
        if (!SDKUtil.isEmpty(value)) {
            this.exponent = value.trim();
        }
    }
}