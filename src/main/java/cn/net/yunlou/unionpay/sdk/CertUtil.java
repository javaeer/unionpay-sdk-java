...
public class CertUtil {
     ...
     * @param path 密钥库路径
     * @param pwd 密钥库密码
     * @return 私钥对象，获取失败返回null
     */
    private static Cert addSignCert(String path, String pwd) {

        if (SDKUtil.isEmpty(path) || SDKUtil.isEmpty(pwd)) {
	 ...
	 * 重置敏感信息加密证书公钥。
	 * @param strCert 证书字符串
	 * @return 更新结果，1-更新成功，0-无需更新，-1-更新失败
	 */
	public static int resetEncryptCertPublicKey(String strCert) {
        ...
        if(SDKUtil.isEmpty(localCertPath)){
...
//    }
	 ...
	 * 
	 * @param x509CertString 证书字符串
	 * @return X509Certificate对象，转换失败返回null
	 */
	 ...
	 * 验证书链。
	 * @param cert 待验证的证书
	 * @param middleCert 中间证书
	 * @param rootCert 根证书
	 * @return 验证结果，验证成功返回true，失败返回false
	 */
